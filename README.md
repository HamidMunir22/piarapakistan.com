...
## 🆕 Phase 7 — Security, Verification & Design Refresh (Latest)

This update focused on hardening account security, fixing the reported OTP delivery issue, tightening the
seller/shop verification flow, and refreshing the site's design and mobile experience. Everything below is
already live in this package.

**Security**
- **Login lockout:** after 3 wrong password attempts, an account is locked for 30 minutes (configurable via
  `MAX_LOGIN_ATTEMPTS` / `LOCK_MINUTES` in `backend/.env`).
- **reCAPTCHA:** Register and Login both support Google reCAPTCHA v2. If you haven't created keys yet at
  https://www.google.com/recaptcha/admin, it's automatically skipped (no keys = no blocking) — add
  `RECAPTCHA_SECRET_KEY` to `backend/.env` and `VITE_RECAPTCHA_SITE_KEY` to `frontend/.env` once you have them.
- Passwords remain hashed (bcrypt), sensitive fields (CNIC, bank account number) are still stripped from every
  API response, and rate limiting + helmet remain on the API.

**OTP delivery fix ("code nahi aata mobile pr")**
- The root cause: without a real, Pakistan-capable SMS provider configured on Railway, `sendSMS()` silently
  falls back to "dev mode" (just a server log) — the SMS is never actually sent.
- Fix: every OTP is now **also emailed automatically** as a guaranteed fallback, so registration/login never gets
  stuck even if SMS delivery isn't configured yet. For real SMS delivery, double-check your Twilio number can
  actually send to `+92` numbers (many trial numbers can't) — see the comments in `backend/.env.example`.

**Seller/Shop verification (KYC)**
- **Buyers**: no CNIC/ID card required at all; profile picture is optional.
- **Sellers/Shops**: must accept Terms & Conditions first — only then does the form reveal the ID card
  front/back **and a selfie holding the ID card** (all three are mandatory).
- New seller/shop accounts go into `kycStatus: "pending"` immediately and stay held (cannot list) until an admin
  reviews their documents — usually resolved within 24 hours. The moment an admin approves or rejects (even 5
  minutes after signup), the user gets an **instant email + SMS notification** — it doesn't wait for the 24 hours.
- Welcome email is sent on every signup, and category now supports **"Other"** with a free-text custom name for
  services/products not in the fixed category list.

**Payments / payout**
- Sellers/shops can now select from a full list of Pakistani banks (HBL, UBL, MCB, Meezan, etc.) plus
  JazzCash/Easypaisa for their payout details — not limited to just JazzCash/Easypaisa. See `backend/utils/banks.js`.

**Design & frontend**
- Adopted a cleaner layout: hero section, "Why Choose Us" cards, a full multi-column **Footer**, and dedicated
  **About** and **Contact** pages (separate from the complaint-focused Help Center).
- **Mobile navigation fixed** — the navbar now uses a proper hamburger menu on small screens that pushes content
  down instead of overlapping it.
- Added a floating **FAQ chatbot** widget (bottom-right, all pages), sign-up/login card entrance animations, and
  kept the existing 3D tilt effects on cards plus the 3D floating hero scene on the homepage.
- **Default language is English** (site was defaulting to Urdu in `index.html`'s `lang` attribute — fixed). The
  language toggle in the navbar still switches to Urdu on demand.
- Added `/terms` and `/privacy` pages — both are plain, editable placeholders ready for you to extend with more
  clauses later.

**Setup checklist for the owner**
1. Copy `backend/.env.example` → `backend/.env` and fill in `MONGO_URI`, `JWT_SECRET`, SMTP credentials.
2. (Optional but recommended) Create Google reCAPTCHA keys and add them to both `.env` files.
3. Verify your Twilio (or alternative) SMS number can send to Pakistani numbers — otherwise OTPs will only
   arrive by email, which still works but isn't ideal long-term.
4. Copy `frontend/.env.example` → `frontend/.env` and set `VITE_API_URL` to your backend's live URL.
